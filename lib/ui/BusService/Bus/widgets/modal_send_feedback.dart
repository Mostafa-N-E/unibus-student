...
class SmallScreenBusReactionWidget extends StatefulWidget {
  TurnEntity turn;
  final Function() notifyParent;
  SmallScreenBusReactionWidget({Key? key, required this.turn, required this.notifyParent}) : super(key: key);

  @override
  State<SmallScreenBusReactionWidget> createState() => _SmallScreenBusReactionWidgetState();
}

class _SmallScreenBusReactionWidgetState extends State<SmallScreenBusReactionWidget> {
  ...
  void initState() {
    ...
    setState((){
      report_int = widget.turn.get_reaction(global_userInfo.id);
      if(report_int != -1){
              ...
              // mainAxisSize: MainAxisSize.min,
                children: [
                  ...
                  ResponsiveLayout.isMobileSmall(context) ? Center() : Padding(
                    ...
                    child: Text(
                        "لطفا نظر خود را درباره این سرویس با انتخاب یکی از گزینه های زیر اعلام کنید",
                        maxLines: 3,
                        textAlign: TextAlign.center,
                        style: TextStyle(
                          overflow: TextOverflow.ellipsis,
                  ...
                  ResponsiveLayout.isMobileSmall(context) ? Center() : Padding(
                    padding: EdgeInsets.only(top: 0, right: 10, left: 10, bottom: 0),
                    child: Text(
                        "با شرکت در این نظز سنجی ما را در ارتقا کیفیت سرویس ها و ارزیابی بهتر یاری میکنید",
                        maxLines: 3,
                        textAlign: TextAlign.center,
                        style: TextStyle(
                          overflow: TextOverflow.ellipsis,
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async{
                      ...
                      turnRepository.record_feedback({
                        ...
                        setState(() {
                          if (fetch_data['status'] == true){
                            ...
                            if(is_change_vote == false){
                              switch (report_int) {
                                case 0:
                                  widget.turn.like.add(global_userInfo.id);
                                  break;
                                case 1:
                                  widget.turn.refused.add(global_userInfo.id);
                                  break;
                                case 2:
                                  widget.turn.dislike.add(global_userInfo.id);
                                  break;
                              }
                            }else{
                              switch (previous_vote) {
                                case 0:
                                  widget.turn.like.remove(global_userInfo.id);
                                  break;
                                case 1:
                                  widget.turn.refused.remove(global_userInfo.id);
                                  break;
                                case 2:
                                  widget.turn.dislike.remove(global_userInfo.id);
                                  break;
                              }
                              switch (report_int) {
                                case 0:
...
class LargeScreenBusReactionWidget extends StatefulWidget {
  TurnEntity turn;
  final Function() notifyParent;
  LargeScreenBusReactionWidget({Key? key, required this.turn, required this.notifyParent}) : super(key: key);

  @override
  State<LargeScreenBusReactionWidget> createState() => _LargeScreenBusReactionWidgetState();
}

class _LargeScreenBusReactionWidgetState extends State<LargeScreenBusReactionWidget> {
  // Map profile_data = Hive.box('cache').get('profile_data', defaultValue: {});
  int _selectedIndex = 2;
  String send_button_text = "ارسال";
  bool is_change_vote = false;
