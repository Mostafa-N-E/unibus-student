...
class LargeReserveBusPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ReserveBusLargeScreenAppBar app_bar = ReserveBusLargeScreenAppBar(context);
    ...
    return BlocProvider(
      ...
      child: BlocBuilder<ReserveBusBloc, ReserveBusState>(builder: ((context, state) {
        if (state is ReserveBusSuccess) {
          Map<String, List<TurnEntity>> classified_turns = TurnEntity.get_turns_dased_on_type(state.turns);
          return Scaffold(
                ...
                children: [
                  Expanded(
                    child: Container(
                      ...
                      child: classified_turns['went']?.length != 0 ?
                      ListView.builder(
                          itemCount: classified_turns['went']?.length,
                          padding: const EdgeInsets.fromLTRB(12, 12, 12, 100),
                          itemBuilder: (context, index) {
                            return LargePageBusReserveCard(
                              turn: classified_turns['went']![index],
                            );
                          }
                      )
                          : Container(
                  ...
                  Expanded(
                    child: Container(
                      width: width * 0.6,
                      child: classified_turns['returns']?.length != 0 ?
                      ListView.builder(
                          itemCount: classified_turns['return']?.length,
                          padding: const EdgeInsets.fromLTRB(12, 12, 12, 100),
                          itemBuilder: (context, index) {
                            return LargePageBusReserveCard(
                              turn: classified_turns['return']![index],
                            );
                          }
                        )
                      :
          ...
          return Scaffold(
            appBar: app_bar.build(
                dormitory_selecter_onTap:  () async {
                  ...
                  BlocProvider.of<ReserveBusBloc>(context).add(ReserveBusChangeSelfService(picked_date: state.picked_date as Jalali, dormitory_id: state.dormitory_id, dormitories: state.dormitoies));
                },
                date_piker_onTap:  () async {
                  Jalali? picked = await showPersianDatePicker(
                    context: context,
                ...
                date_lable:state.picked_date.formatFullDate(),
                dormitory_lable: state.dormitory_id != -1 ? state.dormitoies[state.dormitory_id].name : "-------"
            ),
            floatingActionButton: floating_action_button.build(),
            body: Center(),
          );
